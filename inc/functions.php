...
function validate_game(array $d): array {
  $errors=[];
  $title=trim($d['title']??''); $platform=trim($d['platform']??''); $cover=trim($d['cover_url']??'');
  if($title===''||mb_strlen($title)>120) $errors['title']='Title is required (max 120).';
  if($platform===''||mb_strlen($platform)>40) $errors['platform']='Platform is required (max 40).';
  if($cover!=='' && !filter_var($cover,FILTER_VALIDATE_URL)) $errors['cover_url']='Cover must be a URL.';
  return [$errors,[
